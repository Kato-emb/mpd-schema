...
- [0009](./0009-resolve-v1-scope-segment-enumeration-static-dynamic.md) — **mpd-resolve v1 はセグメント列挙4機能 + dynamic 可用ウィンドウ計算** (accepted): BaseURL 解決・テンプレート展開・タイムライン展開・セグメント列挙。dynamic は now（`SystemTime`）を必須引数に取る別エントリポイントで対応し、低レイテンシ系はエラーで拒否。
...
- [0011](./0011-resolve-public-api-indices-iterator-url-alternatives.md) — **公開 API はインデックス指定 + 検証済み列挙 + 全代替の `Vec<String>`** (accepted): `&Mpd` + (period, adaptation_set, representation) 指定、複数 BaseURL は全代替を順序契約付きで列挙、相対参照のままの出力を許す。
